...
// PIKO-only, unlike the mining site's Wallet.tsx -- this site only ever
// bets PIKO (see Dice.tsx), so there's no reason to carry ICP send/receive
// here too. Exists so a player who's just logged in can actually get PIKO
// into their own principal to bet with, without leaving this site.
export function Wallet({ identity }: WalletProps) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      const piko = await getLedgerActor().icrc1_balance_of({ owner });
  ...
  async function handleSend(event: React.FormEvent) {
    ...
    setSendStatus(null);
    try {
      const result = (await getLedgerActor(identity).icrc1_transfer({
        to: { owner: toPrincipal },
        amount,
      })) as { Ok: bigint } | { Err: Record<string, unknown> };
      if ("Ok" in result) {
        setSendStatus(`Sent ${formatPiko(amount)} PIKO.`);
        setSendTo("");
        setSendAmount("");
        refresh();
      } else {
  ...
  return (
    <section className="block wallet-panel">
      ...
      <div className="wallet-receive">
        <div className="stat-label">Your address -- send PIKO here before you can bet</div>
      ...
      <form className="wallet-send" onSubmit={handleSend}>
        ...
        <p className="wallet-hint">
          Principal: {shortPrincipal(principalText)} -- transfers use the standard ICRC-1 ledger fee
          (0.0001 PIKO).
        </p>
      </form>
    </section>
